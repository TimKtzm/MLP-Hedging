...
from ANN_func import build_ANN, history, pred_X 

os.chdir('/Users/timothee/Documents/IESEG/4a/Master Thesis/Data_Management_HR_impl')

df_op = pd.read_csv('ready_for_ANN_full.csv')
df_op = df_op[['delta', 'M', 'Time_to_expiry', 'impl_volatility', 'cp_int']]

columns_to_check = ['delta', 'M', 'Time_to_expiry', 'impl_volatility', 'cp_int']
...
model = build_ANN(X_test, num_units = 40, dropout_rate = 0.25, loss= 'mse')
...
delta_pred = pred_X(model, X_test, X_index)
delta_pred.to_csv('delta_pred.csv', index=True)  
